...
public class AssetPanel extends JPanel implements MouseListener, ActionListener {
  ...
  public final void updateInfo() {
    ...
    if (info != null) {
      ...
      volumeLabel.setText(formatDouble(info.getVolume() / 1000000) + "M");
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource().equals(clearItem)) {
      ...
      final String input = JOptionPane.showInputDialog(
                    MainFrame.getMainComponent(),
                    "Enter new symbol",
                    "Add",
                    JOptionPane.QUESTION_MESSAGE);
      if(input != null && !input.isBlank()) {
        final AssetInfo info = AssetService.getInstance().getAssetInfo(input);

        if(info != null) {
          final Asset newAsset = new Asset(input.toUpperCase(), info.getName(), order);

          // Check to see if it already exists
          if(!assetGroup.getAssets().contains(newAsset)) {
            update(newAsset);
